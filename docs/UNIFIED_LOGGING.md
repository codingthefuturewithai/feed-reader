...
The feed_reader implements a unified logging system that captures both tool executions and internal application logs in a single, queryable database. The system features:
...
```python
from feed_reader.log_system import set_correlation_id, get_correlation_id
...
```python
from feed_reader.log_system import get_tool_logger
from mcp.server.fastmcp import Context
...
```python
from feed_reader.decorators import tool_logger
from mcp.server.fastmcp import Context
...
```python
from feed_reader.log_system import CorrelationContext, get_tool_logger
...
```python
from feed_reader.log_system.destinations import SQLiteDestination
