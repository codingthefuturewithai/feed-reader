# MCP Inspector Testing Guide for feed_reader
...
which mcp
# Should show: /path/to/feed_reader/.venv/bin/mcp
...
1. Use the full path to your venv's mcp:
   ```bash
   .venv/bin/mcp dev feed_reader/server/app.py
   ```
...
```bash
mcp dev feed_reader/server/app.py
```
...
```bash
python -m feed_reader.server.app --transport sse
```
...
```bash
python -m feed_reader.server.app --transport streamable-http
```
...
- macOS: `~/Library/Application Support/feed_reader/tool_logs.db`
- Linux: `~/.local/share/feed_reader/tool_logs.db`
- Windows: `%LOCALAPPDATA%\feed_reader\tool_logs.db`
...
If you see `ModuleNotFoundError: No module named 'feed_reader'`:

1. Ensure you're in the project root directory
2. Check virtual environment is activated: `which python`
3. Reinstall: `uv sync`
4. Use full mcp path: `.venv/bin/mcp dev feed_reader/server/app.py`
...
1. Add your own tools in `feed_reader/tools/`
