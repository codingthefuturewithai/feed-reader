...
```python
from feed_reader.tools.my_tools import my_tools, my_parallel_tools
```
...
```bash
# Start the inspector
uv run mcp dev feed_reader/server/app.py
...
uv run pytest --cov=feed_reader --cov-report=html
