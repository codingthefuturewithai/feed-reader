...
3. **Verify installation:**
   ...
   feed_reader-server --transport stdio
...
```python
from feed_reader.log_system.destinations.factory import create_destination
...
2. **Register tool in server:**
   ```python
   # server/app.py
   from feed_reader.tools.my_tool import my_tool
...
# tests/conftest.py
import pytest
from feed_reader.config import ServerConfig
...
# Check formatting
black --check feed_reader/
isort --check-only feed_reader/
...
# View logs
tail -f ~/.local/state/mcp-servers/logs/feed_reader.log  # Linux
tail -f ~/Library/Logs/mcp-servers/feed_reader.log       # macOS
...
- Use absolute imports: `from feed_reader.tools import my_tool`
