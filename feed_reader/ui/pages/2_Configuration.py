"""Configuration management page for feed_reader Admin UI
...
try:
    from feed_reader.ui.lib.components import (
        render_info_card,
        render_warning_banner,
        render_config_section
    )
    from feed_reader.ui.lib.utils import (
        load_configuration,
...
def render_configuration_form(config):
    ...
    with st.form("config_form"):
        ...
        with col1:
            ...
            server_name = st.text_input("Server Name", value=server_config.get("name", "feed_reader"))
            server_port = st.number_input("Server Port", 
                                        value=server_config.get("port", 3001),
                                        min_value=1, max_value=65535)
            # Get current log level from runtime config
            try:
                from feed_reader.config import get_config
...
def handle_save_config(server_name, server_port, log_level, log_retention, current_config):
    ...
    new_config = {
        "server": {
            "name": server_name,
            "description": current_config.get("server", {}).get("description", "MCP server for RSS feed management with read/unread tracking"),
            "port": server_port,
            "log_level": log_level,
...
def handle_export_config(config):
    ...
    st.download_button(
        ...
        data=config_json,
        file_name="feed_reader_config.json",
...
def render_environment_variables():
    ...
    env_vars = {
        "FEED_READER_LOG_LEVEL": "INFO",
        "FEED_READER_PORT": "3001",
        "FEED_READER_CONFIG_PATH": system_info.get("config_path", "~/.config/feed_reader"),
        "FEED_READER_LOG_PATH": system_info.get("log_path", "~/.local/share/feed_reader"),
...
def main():
    ...
    # Page header
    st.title("⚙️ feed_reader Configuration")
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=config_yaml,
            file_name="feed_reader_config.yaml",
