...
def get_sqlite_logs(correlation_id: str) -> List[Dict]:
    ...
    import platformdirs
    app_data = platformdirs.user_data_dir("feed_reader")
...
async def test_all_tools(server_script_path: str):
    ...
    server_params = StdioServerParameters(
        command="python",
        args=["-m", "feed_reader.server.app", "--transport", "stdio"],
        env=env
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            console.print("1. Run the Streamlit UI: [cyan]streamlit run feed_reader/ui/app.py[/cyan]")
            ...
            db_path = Path.home() / ".local" / "share" / "feed_reader" / "unified_logs.db"
...
@click.option(
    '--server-module',
    default="feed_reader.server.app",
