#!/usr/bin/env python3
"""Feed Reader Docker Manager

Cross-platform script to manage the Feed Reader MCP server Docker container.
...
IMAGE_NAME = "feed-reader-mcp"
BASE_PORT = 19000
CONFIG_DIR = Path.home() / ".config" / "feed_reader"
...
def start_container(port: Optional[int] = None) -> bool:
    ...
    code, _, stderr = run_command([
        ...
        "-v", "feed_reader_data:/home/appuser/.feed_reader",
        "-v", "feed_reader_config:/home/appuser/.config/feed_reader",
        "-v", "feed_reader_logs:/home/appuser/.local/share/feed_reader",
...
def cmd_start():
    ...
    print(f"\n{Colors.BOLD}{Colors.GREEN}Feed Reader MCP - Start{Colors.RESET}\n")
    ...
    print_success("Feed Reader MCP server is running!")
    print()
    print_info(f"MCP endpoint: http://localhost:{port}/mcp")
    print()
    print_info("Add to Claude Code:")
    print(f"  {Colors.CYAN}claude mcp add feed-reader --transport http http://localhost:{port}/mcp{Colors.RESET}")
    print()
...
def cmd_stop():
    """Stop the container"""
    print(f"\n{Colors.BOLD}{Colors.GREEN}Feed Reader MCP - Stop{Colors.RESET}\n")
    stop_container()


def cmd_restart():
    """Restart the container (without rebuild)"""
    print(f"\n{Colors.BOLD}{Colors.GREEN}Feed Reader MCP - Restart{Colors.RESET}\n")
...
def cmd_update():
    ...
    print(f"\n{Colors.BOLD}{Colors.GREEN}Feed Reader MCP - Update{Colors.RESET}")
    ...
    print_success("Feed Reader MCP server updated with latest code!")
    print()
    print_info(f"MCP endpoint: http://localhost:{port}/mcp")
    print()
    print_info("Useful commands:")
...
def cmd_status():
    ...
    print(f"\n{Colors.BOLD}{Colors.GREEN}Feed Reader MCP - Status{Colors.RESET}\n")
...
def cmd_logs():
    """Tail container logs"""
    print(f"\n{Colors.BOLD}{Colors.GREEN}Feed Reader MCP - Logs{Colors.RESET}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Feed Reader Docker Manager",
