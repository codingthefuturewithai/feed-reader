...
feed_reader-server --transport sse --port 3001

# Start with streamable HTTP transport
feed_reader-server --transport streamable-http --port 3001

# Get help
feed_reader-server --help
```

### Testing Commands
```bash
# Test client (stdio)
feed_reader-client "Hello, World!"

# Run all tests
pytest

# Run with coverage
pytest --cov=feed_reader
...
PYTHONPATH=. mcp dev feed_reader/server/app.py

# Streamlit UI (monitoring interface)
streamlit run feed_reader/ui/app.py
...
echo '{"method": "tools/list"}' | feed_reader-server
```
...
# View logs
# macOS:
tail -f ~/Library/Logs/mcp-servers/feed_reader.log
# Linux:
tail -f ~/.local/state/mcp-servers/logs/feed_reader.log
# Windows PowerShell:
Get-Content -Wait "$env:LOCALAPPDATA\mcp-servers\logs\feed_reader.log"
```

### Transport Debugging
```bash
# Test stdio (default)
echo '{"method": "ping"}' | feed_reader-server
...
# Windows PowerShell:
Start-Job { feed_reader-server --transport sse --port 3001 }
...
# Windows PowerShell:
Start-Job { feed_reader-server --transport streamable-http --port 3001 }
...
- **MCP_SERVER_NAME**: Custom server name (default: "feed_reader")
...
2. ✅ Test with `feed_reader-client "test"`
...
This setup guide should get you productive with feed_reader quickly!