...
IMPORTANT: Tests use isolated temporary databases via FEED_READER_DB_PATH env var.
Production database (~/.feed_reader/feed_reader.db) is NEVER touched by tests.
"""
...
# =============================================================================
# Each test session gets a unique temporary database directory.
# This ensures tests NEVER touch the production database.

_test_db_dir: Optional[Path] = None


def _get_test_db_path() -> Path:
    """Get or create the test database path for this session."""
    global _test_db_dir
    if _test_db_dir is None:
        _test_db_dir = Path(tempfile.mkdtemp(prefix="feed_reader_test_"))
    return _test_db_dir / "test_feed_reader.db"
...
def test_database_isolation():
    """Session-scoped fixture to ensure test database isolation.

    Sets FEED_READER_DB_PATH environment variable to a temporary location
    before any tests run, and cleans up afterward.
    """
    test_db_path = _get_test_db_path()
    original_env = os.environ.get("FEED_READER_DB_PATH")

    # Set the env var for the test session
    os.environ["FEED_READER_DB_PATH"] = str(test_db_path)

    yield test_db_path

    # Restore original env var
    if original_env is not None:
        os.environ["FEED_READER_DB_PATH"] = original_env
    else:
        os.environ.pop("FEED_READER_DB_PATH", None)
...
async def clean_database_between_tests(test_database_isolation):
    """Clean the test database before each test for isolation.

    This ensures each test starts with a fresh, empty database.
    """
    from feed_reader.storage.database import close_database, _get_db_path
...
class StreamableHTTPServer:
    ...
    def __init__(self, port: int = 3001):
        ...
        self.server_module = "feed_reader.server.app"
    ...
    def start(self) -> None:
        ...
        # Build environment - MUST include FEED_READER_DB_PATH for test isolation
        env = os.environ.copy()
        env["PYTHONPATH"] = str(self.project_root)
        # Ensure test database isolation is passed to subprocess
        if "FEED_READER_DB_PATH" not in env:
            env["FEED_READER_DB_PATH"] = str(_get_test_db_path())
...
async def mcp_session(request) -> AsyncGenerator[Tuple[ClientSession, str], None]:
    ...
    try:
        if transport == "stdio":
            ...
            server_module = "feed_reader.server.app"
            ...
            # CRITICAL: Pass test database path to subprocess for isolation
            if "FEED_READER_DB_PATH" in os.environ:
                env["FEED_READER_DB_PATH"] = os.environ["FEED_READER_DB_PATH"]
            else:
                env["FEED_READER_DB_PATH"] = str(_get_test_db_path())
